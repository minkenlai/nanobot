...
from nanobot.channels import telegram as _tg_mod
...
def _forum_topic_created_update(
    chat_id: int = -100,
    thread_id: int = 5,
    topic_name: str = "General",
) -> SimpleNamespace:
    """Simulate a FORUM_TOPIC_CREATED update."""
...
def _forum_topic_edited_update(
    chat_id: int = -100,
    thread_id: int = 5,
    topic_name: str = "Renamed Topic",
) -> SimpleNamespace:
    """Simulate a FORUM_TOPIC_EDITED update."""
    topic_edit = SimpleNamespace(name=topic_name)
    message = SimpleNamespace(
        chat_id=chat_id,
        message_thread_id=thread_id,
...
def mock_persistence():
    """Mock topic pins persistence to use an in-memory dict per test."""
    _store: dict = {}
    ...
    with (
        patch.object(_tg_mod, "load_topic_pins", _load),
        patch.object(_tg_mod, "save_topic_pins", _save),
    ):
        yield _store
...
async def test_on_pin_event_valid_profile():
    ...
    assert ch._topic_pins["-100:5"] == (42, "fast", None)
...
async def test_on_pin_event_non_profile_message():
    ...
    await ch._on_pin_event(update, None)
    assert ch._topic_pins["-100:5"] == (7, None, None)


async def test_on_pin_event_unpin_clears_cache():
    ch = _make_channel()
    # Seed the cache first
    ch._topic_pins["-100:5"] = (42, "fast", None)
...
async def test_on_edited_message_updates_pinned_profile():
    ...
    assert ch._topic_pins["-100:5"] == (42, "deep", None)
...
async def test_on_edited_message_clears_profile_if_text_changed():
    ...
    await ch._on_edited_message(update, None)
    assert ch._topic_pins["-100:5"] == (42, None, None)
...
async def test_on_edited_message_ignores_non_pinned_message():
    ...
    await ch._on_edited_message(update, None)
    assert ch._topic_pins["-100:5"] == (42, "fast", None)  # unchanged
...
async def test_get_topic_profile_pin_cold_start_seeds_cache():
    ...
    assert ch._topic_pins["-100:5"] == (42, "balanced", None)
...
async def test_get_topic_profile_pin_cold_start_no_pin():
    ...
    assert ch._topic_pins["-100:5"] == (-1, None, None)
...
async def test_get_topic_profile_pin_lazy_fill_then_discover_native():
    ...
    ch._topic_pins["-100:5"] = (None, None, None)
...
# ---------------------------------------------------------------------------
# _on_forum_topic_created
# ---------------------------------------------------------------------------


async def test_on_forum_topic_created_captures_name():
    ch = _make_channel()
    update = _forum_topic_created_update(topic_name="Design Reviews")
    await ch._on_forum_topic_created(update, None)
    assert ch._topic_pins["-100:5"] == (None, None, "Design Reviews")


async def test_on_forum_topic_created_ignores_no_message():
    ch = _make_channel()
    update = SimpleNamespace(message=None)
    await ch._on_forum_topic_created(update, None)
    assert ch._topic_pins == {}
...
async def test_on_forum_topic_created_ignores_no_topic_object():
    ch = _make_channel()
    message = SimpleNamespace(
        chat_id=-100,
        message_thread_id=5,
...
async def test_on_forum_topic_created_ignores_no_thread_id():
    ch = _make_channel()
    topic = SimpleNamespace(name="General")
    message = SimpleNamespace(
        chat_id=-100,
...
# ---------------------------------------------------------------------------
# _on_forum_topic_edited
# ---------------------------------------------------------------------------


async def test_on_forum_topic_edited_updates_name():
    ch = _make_channel()
    update = _forum_topic_edited_update(topic_name="Engineering")
    await ch._on_forum_topic_edited(update, None)
    assert ch._topic_pins["-100:5"] == (None, None, "Engineering")


async def test_on_forum_topic_edited_ignores_no_message():
    ch = _make_channel()
    update = SimpleNamespace(message=None)
    await ch._on_forum_topic_edited(update, None)
    assert ch._topic_pins == {}
...
async def test_on_forum_topic_edited_ignores_no_topic_object():
    ch = _make_channel()
    message = SimpleNamespace(
        chat_id=-100,
        message_thread_id=5,
...
async def test_on_forum_topic_edited_ignores_no_thread_id():
    ch = _make_channel()
    topic_edit = SimpleNamespace(name="New Name")
    message = SimpleNamespace(
        chat_id=-100,
        message_thread_id=None,
        forum_topic_edited=topic_edit,
...
# ---------------------------------------------------------------------------
# Combined flows: topic name + profile pin
# ---------------------------------------------------------------------------


async def test_combined_create_topic_then_pin_preserves_topic_name():
    """Create topic, then pin profile — topic_name should be preserved in 3-tuple."""
    ch = _make_channel()

    # 1. Create forum topic
    update = _forum_topic_created_update(thread_id=5, topic_name="Design Reviews")
    await ch._on_forum_topic_created(update, None)
    assert ch._topic_pins["-100:5"] == (None, None, "Design Reviews")

    # 2. Pin a profile message
    update = _pin_update(thread_id=5, pinned_text="Profile: fast", pinned_message_id=42)
    await ch._on_pin_event(update, None)
    assert ch._topic_pins["-100:5"] == (42, "fast", "Design Reviews")


async def test_combined_pin_then_create_topic_preserves_profile():
    """Pin profile first, then create topic — both profile and topic_name should be set."""
    ch = _make_channel()

    # 1. Pin a profile message (topic_name is None at this point)
    update = _pin_update(thread_id=5, pinned_text="Profile: deep", pinned_message_id=10)
    await ch._on_pin_event(update, None)
    assert ch._topic_pins["-100:5"] == (10, "deep", None)

    # 2. Create forum topic — preserves existing profile, sets topic_name
    update = _forum_topic_created_update(thread_id=5, topic_name="Engineering")
    await ch._on_forum_topic_created(update, None)
    assert ch._topic_pins["-100:5"] == (10, "deep", "Engineering")


async def test_combined_topic_rename_preserves_pinned_profile():
    """Rename topic — topic_name updates but pinned profile should be unaffected by rename alone."""
    ch = _make_channel()

    # 1. Create topic
    update = _forum_topic_created_update(thread_id=5, topic_name="General")
    await ch._on_forum_topic_created(update, None)
    assert ch._topic_pins["-100:5"] == (None, None, "General")

    # 2. Pin a profile
    update = _pin_update(thread_id=5, pinned_text="Profile: balanced", pinned_message_id=99)
    await ch._on_pin_event(update, None)
    assert ch._topic_pins["-100:5"] == (99, "balanced", "General")

    # 3. Rename topic — preserves existing profile
    update = _forum_topic_edited_update(thread_id=5, topic_name="Design Reviews")
    await ch._on_forum_topic_edited(update, None)
    assert ch._topic_pins["-100:5"] == (99, "balanced", "Design Reviews")


async def test_combined_full_lifecycle_all_three_elements():
    """Full lifecycle: create topic → pin profile → edit pin → rename topic → re-pin."""
    ch = _make_channel()

    # 1. Create topic
    await ch._on_forum_topic_created(
        _forum_topic_created_update(thread_id=7, topic_name="Backend"), None
    )
    assert ch._topic_pins["-100:7"] == (None, None, "Backend")

    # 2. Pin profile
    await ch._on_pin_event(
        _pin_update(thread_id=7, pinned_text="Profile: nano", pinned_message_id=200), None
    )
    assert ch._topic_pins["-100:7"] == (200, "nano", "Backend")

    # 3. Edit pinned message to change profile
    await ch._on_edited_message(
        _edit_update(thread_id=7, message_id=200, text="Profile: deep"), None
    )
    assert ch._topic_pins["-100:7"] == (200, "deep", "Backend")

    # 4. Rename topic — preserves existing profile
    await ch._on_forum_topic_edited(
        _forum_topic_edited_update(thread_id=7, topic_name="Infrastructure"), None
    )
    assert ch._topic_pins["-100:7"] == (200, "deep", "Infrastructure")

    # 5. Re-pin a profile
    await ch._on_pin_event(
        _pin_update(thread_id=7, pinned_text="Profile: fast", pinned_message_id=300), None
    )
    assert ch._topic_pins["-100:7"] == (300, "fast", "Infrastructure")
