...
async def test_send_clears_typing_after_send() -> None:
    ...
    await channel.send(
        OutboundMessage(
            address=Address(channel="matrix", segments=("!room:matrix.org",)), content="Hi"
        )
    )
...
async def test_send_adds_formatted_body_for_markdown() -> None:
    ...
    await channel.send(
        OutboundMessage(
            address=Address(channel="matrix", segments=("!room:matrix.org",)), content=markdown_text
        )
    )
...
async def test_send_keeps_plaintext_only_for_plain_text() -> None:
    ...
    await channel.send(
        OutboundMessage(
            address=Address(channel="matrix", segments=("!room:matrix.org",)), content=text
        )
    )
